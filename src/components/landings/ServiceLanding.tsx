...
import LandingCampaignHeader from "components/landings/LandingCampaignHeader";
...
const AUTHORITY_LINE = "Pablo Roberto · Advogado com atuação em Direito de Trânsito · Belém – PA";
const PHONE_LABEL = "+55 91 98100-6131";
...
const CTA_CLASS_FOCUSED =
  "inline-flex min-h-[48px] w-full items-center justify-center rounded-full bg-[#25D366] px-5 py-3 text-base font-bold text-white shadow-lg transition hover:brightness-110 sm:w-auto sm:px-8 sm:py-4 sm:text-lg";
...
function WhatsAppCta({
  ...
  return (
    <a
      ...
      className={`${focused ? CTA_CLASS_FOCUSED : CTA_CLASS} ${className}`}
    >
      {label}
    </a>
  );
...
function ProfessionalCard({ priority = false }: { priority?: boolean }) {
  return (
    <>
...
export default function ServiceLanding({ menu, content, variant = "default" }: ServiceLandingProps) {
  const isFocused = variant === "focused";
  const canonicalUrl = `${BASE_URL}/${content.slug}`;
  const whatsappHref = whatsappLink(content.whatsappMessage);
  const ctaLabel = isFocused ? content.ctaLabelFocused : content.ctaLabel;
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-blue-950 text-white">
        <Analytics />
        {isFocused ? (
          <LandingCampaignHeader logoUrl={menu?.logoUrl} />
        ) : (
        ...
        <main>
          <section
            className={`relative overflow-hidden ${
              isFocused ? "pt-[4.25rem] pb-8 sm:pt-20 md:pt-24 md:pb-14" : "pt-24 pb-10 sm:pt-28 md:pt-32 md:pb-16"
            }`}
            id="inicio"
          >
            ...
            <div className="relative mx-auto max-w-7xl px-5 sm:px-6 md:px-10">
              ...
              <div className="grid grid-cols-1 items-center gap-6 md:grid-cols-2 md:gap-12">
                <div className={`flex flex-col ${isFocused ? "gap-3" : "gap-4"}`}>
                  ...
                  {isFocused && <p className="text-sm font-medium text-[#fec655]">{AUTHORITY_LINE}</p>}

                  <WhatsAppCta href={whatsappHref} label={ctaLabel} focused={isFocused} />
                  <p className="text-sm text-gray-400">
                    Cada caso deve ser analisado individualmente. O contato não gera obrigação.
                  </p>
                </div>

                <div className={isFocused ? "relative hidden md:block" : "relative"}>
                  <ProfessionalCard priority={!isFocused} />
                </div>
              </div>
            </div>
          </section>
          ...
          {isFocused && (
            <section className="bg-[#070a0f] py-12 md:py-16" id="confianca">
              <div className="mx-auto max-w-3xl px-5 sm:px-6 md:px-10">
                <h2 className="text-center text-2xl font-extrabold text-white md:text-3xl">
                  {content.identificationHeading}
                </h2>
                <p className="mt-4 text-center text-base leading-relaxed text-gray-300 md:text-lg">
                  {content.identificationText}
                </p>
                <div className="relative mx-auto mt-8 max-w-md md:hidden">
                  <ProfessionalCard />
                </div>
                <div className="mt-8 hidden text-center md:block">
                  <p className="text-lg font-extrabold text-white">Pablo Roberto</p>
                  <p className="text-sm font-semibold text-[#fec655]">Advogado com atuação em Direito de Trânsito</p>
                  <address className="mt-2 not-italic text-sm text-gray-400">{ADDRESS}</address>
                  <p className="mt-1 text-sm text-gray-400">WhatsApp: {PHONE_LABEL}</p>
                </div>
              </div>
            </section>
          )}
          ...
          {!isFocused && (content.relatedLinks.length > 0 || content.blogLinks.length > 0) && (
          ...
          <section className="bg-[#fec655] py-12 md:py-16" id="cta-final">
            <div className="mx-auto max-w-4xl px-5 text-center sm:px-6">
              ...
              <WhatsAppCta href={whatsappHref} label={ctaLabel} focused={isFocused} className="mt-8" />
            </div>
          </section>
          ...
          {isFocused ? (
            <LandingLegalFooter whatsappHref={whatsappHref} address={ADDRESS} />
          ) : (
            <Footer menuData={menu} whatsappHref={whatsappHref} />
          )}
        </main>

        <WhatsAppButton href={whatsappHref} showSocial={!isFocused} />
      </div>
    </>
  );
}
